...
- 85a5895: `--engine docker` (and `--engine auto`'s Docker fallback) now works on Apple Silicon. The published Docker image is built multi-arch (linux/amd64 + linux/arm64) instead of amd64-only, so it pulls and runs natively on arm64. Also adds `DRUCK_DOCKER_PLATFORM` to force the container platform (e.g. `linux/amd64`) when relaying to Docker.

## 0.2.3

### Patch Changes

- 78e6130: Ship the bundled templates in the published package. Previously `files` was `["dist"]`, so an npm install had no `base`/`report`/`examples` templates and `druck templates`/`render` found nothing to work with. `templates/` is now published alongside `dist/`, so rendering works from a plain `npm install -g @druckform/core` and not only from the Docker image.
- 5e4893f: Add a `--version` flag to the `druck` CLI. It prints the installed package version and exits; previously `druck --version` printed "unknown".
